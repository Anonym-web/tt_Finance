        ...
        for (String fileName : fileList) {
            ...
            for (StringBuilder line : list) {
                if (line.toString().startsWith("||收入")) {
                    map.put(fileName.split("\\\\", -1)[fileName.split("\\\\", -1).length - 1], line.toString().replace("\r", "").replace("\n", ""));
                    break;
                }
            }
        }
        for (String key : map.keySet()) {
            System.out.println("表头：\t".concat(map.get(key)).concat("\t->\t"));
            System.out.println("文件名：\t".concat(key));
            System.out.println("-------------------------------");
        }
    }

    public static void main(String[] args) throws Exception {
        VerifyFiles.verifyFiles("E:\\2020年1-11月工程业务简表汇总", 0, 94);
    }
}

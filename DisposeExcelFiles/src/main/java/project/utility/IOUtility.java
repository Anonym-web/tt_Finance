...
public class IOUtility {
    ...
    public static BufferedWriter bufferWriter(String outputPath, String codedFormat) throws Exception {
        // 写流（以指定编码格式写入指定文件路径）
        return new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outputPath,true), codedFormat));
    }
    ...
    public static BufferedReader bufferReader(String inputPath, String codedFormat) throws Exception {
        // 读流（指定读取文件路径和编码格式）
        return new BufferedReader(new InputStreamReader(new FileInputStream(inputPath), codedFormat));
    }
}
